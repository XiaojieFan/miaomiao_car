...
通过阅读以下内容可以快速搭建智能战车的硬件软件环境，方便快速上手测试。
...
1. 使用 pwm_led_sample 调试 PWM 时，注释掉 `rt_pin_mode(LED_PIN_NUM , PIN_MODE_OUTPUT) ;  rt_pin_write(LED_PIN_NUM, PIN_HIGH) `; 否则和 pwm 设置会冲突 。
2. 马达的焊线总是容易掉 。
3. 要把驱动板的地和控制板连接，否则嗡嗡小声在响 。
4. 潘多拉板子上PD12 PWM4 1 通道无法输出PWM 波形 ，检查 Kconfig 中相关配置 。
5. 四轮驱动注意电机的顺序 ，左前轮 、右前轮 、左后轮 、 右后轮 。
6. 占空比不能太小，否则连动都不能动， `motor_run(whl->w_motor,600)` 再小就不动了。
